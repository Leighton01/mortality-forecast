...
The effectiveness of the log-linear (LN) and Lee-Carter (LC) models in forecasting mortality trends in Canada is evaluated and compared. Sourcing from the Human Mortality Database (2025), data from 1921 to 2012 are used for training, while data from 2013 to 2022 are used for testing. The analysis focuses the strengths and limitations of each model in capturing patterns and predicting future mortality rates.
...
For instance, the 2016 wildfires in Fort McMurray, Alberta led to temporary but significant increases in deaths, particularly among vulnerable populations like the elderly or those with pre-existing health conditions. These events can skew mortality data in the short term, making it harder to distinguish between the impact of the disaster itself and the usual mortality patterns of the population. Similarly, the COVID-19 pandemic has direct associations with reductions in life expectancy in Canada in 2020, affecting both short-term and long-term forecasts (Dion, 2021). 
...
Another challenge is model overfitting. To mitigate overfitting, regularization techniques like shrinkage priors such as Gaussian priors can be employed, which penalize extreme parameter estimates and help maintain generalizable predictions. Additionally, careful selection of hyperparameters (such as the variance of the priors) may reduce the risk of overfitting. However, this may not be as well suited for when the aforementitioned situations of disaster spikes in mortality.

The computational complexity is another hurdle, especially with high-dimensional data or frequent updates. the process can become computationally expensive. Updating the model each time new data is available can be computationally intensive with methods such as Markov chain Monte Carlo, particularly when dealing with large datasets and multiple sources of uncertainty. 
